...
p <- ggplot() +
  ...
  coord_sf(datum = NA) +
  geom_sf(data = data_country_yearly, aes(geometry = geometry, group = country, size = value), color = "#0090a4" , alpha = 0.85) +
  scale_size_continuous(
    range = c(1, 30) # adapt to aspect ratio
...
p <- ggplot() +
  geom_sf(data = world, color = "lightgray", alpha = 0.7, size = 0.1) +
  coord_sf(datum = NA) +
  geom_sf(data = data_country_decadal, aes(geometry = geometry, group = country, size = value), color = "#0090a4" , alpha = 0.85) +
  scale_size_continuous(
    range = c(1, 20) # adapt to aspect ratio
    , breaks = scales::breaks_log(n = 5, base = 10), labels = addUnits, name = "Production") +
  theme_void() +
  theme(legend.position = "bottom") +
...
animate(p, renderer = av_renderer(tolower(paste('outputs/animation1/animation1', source, paste(area, collapse = "+"), species, format, '.mp4', sep = '_'))), width = ifelse(format == "landscape", 1920, 1080), height = 1080, res = 100, duration = 20, fps = framerate, start_pause = framerate, end_pause = framerate, type = "cairo")
